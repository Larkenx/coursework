...
public class Breakout extends JFrame {
  ...
  private class BreakoutPanel extends JPanel implements KeyListener, ActionListener {
    // Jpanel Variables
    private Timer timer;
    ...
    public BreakoutPanel() {
      ...
      ball = new Ball(FRAME_WIDTH / 2, (int) Math.round(FRAME_HEIGHT * .3), 5, Color.red);
      ball.setSpeed(new Vector(1, 1));
    ...
    private void wonGame() {
      timer.stop();
      int n = JOptionPane.showInternalConfirmDialog(this, "You won! Play again?", "VICTORY!", JOptionPane.YES_NO_OPTION);
      if (n == 0) {
        restartGame();
      } else {
        System.exit(0);
    ...
    private void lostGame() {
      timer.stop();
      int n = JOptionPane.showInternalConfirmDialog(this, "You lost! Play again?", "DEFEAT!", JOptionPane.YES_NO_OPTION);
      if (n == 0) {
        restartGame();
      } else {
        System.exit(0);
    ...
    private void collisions() {
      ...
      if (paddleBounds.intersects(ballBounds)) {
        ...
        for (int i = 0; i < blocks.size(); i++) {
          ArrayList<Block> currentRow = blocks.get(i);
          for (int j = 0; j < currentRow.size(); j ++) { // 10 columns
            Block currentBlock = currentRow.get(j);
            Rectangle blockBounds = currentBlock.getBounds();
            if (blockBounds.intersects(ballBounds)) {
              ...
              if (ballBounds.intersects(topBlock) || ballBounds.intersects(bottomBlock)) {
                ball.setSpeed(new Vector(ballSpeed.getX(), ballSpeed.getY() * -1)); // Reverse verticle velocity
              } else if (ballBounds.intersects(leftBlock) || ballBounds.intersects(rightBlock)) { // Left or Right
                ball.setSpeed(new Vector(ballSpeed.getX() * -1, ballSpeed.getY())); // Reverse horizontal velocity
              }
              // Remove the block
              currentRow.remove(currentBlock);
              if (blocks.isEmpty()) {
                wonGame();
              }
            }
    ...
    public void actionPerformed(ActionEvent e) {
      String result = (String) JOptionPane.showInputDialog(this, "Choose a name for yourself!", "Name Chooser", JOptionPane.PLAIN_MESSAGE, null, null, null);
      if (result != null) {
